<?php
namespace Ttree\Plugin\MicroEvent\Eel\FlowQueryOperations;
 ...
 * Example:
 *
 *    q(node).children().eventCacheLifetime()
 *
 */
class CacheLifetimeOperation extends AbstractOperation
{
     ...
     * @var string
     */
    protected static $shortName = 'eventCacheLifetime';
    ...
    public function evaluate(FlowQuery $flowQuery, array $arguments)
    {
        ...
        foreach ($flowQuery->getContext() as $contextNode) {
            $endDate = $contextNode->getProperty('eventEndDate') ?: $contextNode->getProperty('eventStartDate');
            if ($endDate === null) {
                continue;
            }
            if ($endDate !== null && $endDate > $this->now && ($minimumDateTime === null || $endDate < $minimumDateTime)) {
